...
Dataset       <- c("elevators_all","bank_all", "Year_test","ailerons_all",
                   "calhousing","N-cpusmall","N-parkinsons","N-slice_all") 

savepath1     <- paste0("D:/experiment/JMLR/JMLR2024/Result/",
                         paste0("CONS-sq-",Dataset[d_index],".txt"))
...
for(re in 1:reptimes)
{
  ...
  ### the first instance
  error      <- (ylabel[order[1]])^2+(ylabel[order[2]])^2
  Gram       <- matrix(1,nrow = 1,ncol=1)
  Ut_1       <- matrix(1,nrow = 1,ncol=1)
  ...
  for(t in 2:length_tr)
  {
    if(zt_1 == 1)
    {
      ...
      if(k >1)
      {
        tem1 <- Gramt_1[,1:(num-1)]%*%Ut_1
        Q    <- diag(D)%*%t(U)%*%tem1%*%diag(Dt_1)
        wt   <- Q%*%wt
        A       <- Q%*%A%*%t(Q)
        inver_A <- solve(A+alpha*diag(num))
      }else{
        A       <- alpha*diag(num)
      ...
      tem6    <- inver_A%*%ph_t
      tem5    <- (t(ph_t)%*%tem6)[1,1]
      if(tem5==0)
        tem5 <- 0.00001
      wt      <- vt - sign(z)*max(abs(z)-C,0)/tem5*tem6
    }
    ...
    ########### solving A^-1
    tem7    <- 1+sigma_/2*(t(gt)%*%inver_A%*%gt)[1,1]
    tem8    <- sigma_/2*inver_A%*%gt%*%t(gt)%*%inver_A
    inver_A <- inver_A - tem8/tem7
