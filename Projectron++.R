...
for( re in 1:reptimes)
{
  ...
  for (i in 2:length_tr)
  {
    ...
    if(hatyi == ylabel[order[i]] && ylabel[order[i]]*fx<1)
    {
      #### compute delta
      d_ast <-Inver_K%*%kt
      ...
      if(abs(delta)<1e-8)
        delta <- 0
      if(ylabel[order[i]]*fx < (1-sqrt(delta/eta)))
      {
        tau_t1  <- (1-ylabel[order[i]]*fx)/tem_t
        tau_t2  <- 2*(1-ylabel[order[i]]*fx-sqrt(delta/eta))/tem_t
        tau_t   <- min(tau_t1,tau_t2,1)
        svpara <- svpara + tau_t*ylabel[order[i]]*as.vector(d_ast)
      }
    }
  }
  t2 <- proc.time()
